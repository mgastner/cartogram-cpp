...
void write_labels_to_cairo_surface(cairo_t *cr, const InsetState *inset_state) {
...
void graticule_cell_color(
  ...
  if (area == max_area) {
    ...
    *b = blue[int(xmin)] +
        (blue[int(xmax)] - blue[int(xmin)]) * (category - xmin);
  }
}

std::vector<std::vector<Color_dbl>> graticule_cell_colors(
    InsetState *inset_state,
    unsigned int cell_width) {
  unsigned int lx = inset_state->lx();
  unsigned int ly = inset_state->ly();
  ...
  std::tie(max_area, min_area) =
      max_and_min_graticule_cell_area(inset_state, cell_width);
...
void write_graticules_to_cairo_surface(cairo_t *cr, InsetState *inset_state) {
  ...
  for (unsigned int i = 0; i < lx - graticule_width; i += graticule_width) {
    for (unsigned int j = 0; j < ly - graticule_width; j += graticule_width) {
      ...
      const Polygon cell_edge_points =
          graticule_cell_edge_points(i, j, inset_state);
...
void write_graticule_colors_to_cairo_surface(
  ...
  if (!crop) {
    ...
    for (unsigned int i = 0; i < lx - cell_width; i += cell_width) {
      for (unsigned int j = 0; j < ly - cell_width; j += cell_width) {
        ...
        // Draw graticule cell by connecting edge points
        const Polygon cell_edge_points = graticule_cell_edge_points(
            i,
            j,
            inset_state,
            cell_width,
  ...
  std::vector<GeoDiv> geo_divs = plot_equal_area_map
      ? inset_state->geo_divs_original()
...
void write_map_image(
  ...
  const bool colors = (inset_state->colors_size() == inset_state->n_geo_divs());
  cairo_surface_t *surface;

  // Create a cairo surface
  image_format_ps
      ? surface = cairo_ps_surface_create(filename.c_str(), lx, ly)
      : surface = cairo_svg_surface_create(filename.c_str(), lx, ly);
  ...
  write_polygons_to_cairo_surface(
      cr,
      fill_polygons,
      colors,
      false,
...
Polygon transform_to_albers_coor(
    Polygon edge_points,
...
double graticule_cell_area_km(
    const unsigned int i,
    const unsigned int j,
    InsetState *inset_state) {
  Polygon cell_edge_points =
      graticule_cell_edge_points(i, j, inset_state, 1, true);
  const Polygon cell_edge_points_albers =
      transform_to_albers_coor(cell_edge_points, inset_state);
...
double graticule_cell_target_area(
    ...
    InsetState *inset_state) {
  const Polygon cell_edge_points =
      graticule_cell_edge_points(i, j, inset_state, 1, false);
...
double graticule_cell_target_area_per_km(
    ...
    const double total_inset_area,
    InsetState *inset_state) {
  const double cell_target_area = graticule_cell_target_area(
      i,
      j,
      total_target_area,
      total_inset_area,
...
void write_graticule_heatmap_image(
  ...
  std::tie(max_area_cell_point, min_area_cell_point) =
      max_and_min_graticule_cell_area_index(inset_state, 1);

  const double max_area_cell_point_area = graticule_cell_area(
      max_area_cell_point.x(),
      max_area_cell_point.y(),
      inset_state,
      1);
  const double min_area_cell_point_area = graticule_cell_area(
  ...
  // Draw colors
  write_graticule_colors_to_cairo_surface(
      cr,
      inset_state,
      plot_equal_area_map,
      crop);
...
void write_density_image(
  ...
  double each_graticule_cell_area_km =
      graticule_cell_area_km(0, 0, inset_state);
  ...
  for (unsigned int i = 0; i < inset_state->lx(); ++i) {
    for (unsigned int j = 0; j < inset_state->ly(); ++j) {
      ...
      if (plot_graticule_heatmap) {
        double target_area_per_cell_km =
            density[i * inset_state->ly() + j] / each_graticule_cell_area_km;
  ...
  write_polygons_to_cairo_surface(cr, false, false, false, inset_state);
...
void InsetState::write_intersections_image(
  ...
  write_polygons_to_cairo_surface(cr, false, false, false, this);
