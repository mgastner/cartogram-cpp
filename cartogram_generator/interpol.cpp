                ...
                const unsigned int ly)
{
  ...
  double x0 = std::max(0.0, floor(x + 0.5) - 0.5); // Nearest grid point smaller than x.
                                                   // Exception: if x<0.5, x0 becomes 0.0.
  double x1 = std::min(double(lx), floor(x + 0.5) + 0.5); // Nearest grid point larger than x.
                                                          // Exception: if x>lx-0.5, x1 becomes lx.
  double y0 = std::max(0.0, floor(y + 0.5) - 0.5);        // Similarly for y.
  double y1 = std::min(double(ly), floor(y + 0.5) + 0.5);
  double delta_x = (x - x0) / (x1 - x0); // On a scale from 0 to 1, how far is x (or
  double delta_y = (y - y0) / (y1 / y0); // y) away from x0 (or y0)? 1 means x=x1.
